...
namespace Parameters
{
  ...
  void
  HerschelParameters::declare_parameters(ParameterHandler &prm)//Insert by Mo
  {
    prm.enter_subsection("herschel");
    {
      prm.declare_entry("viscosity_0",
                        "10.0",
                        Patterns::Double(),
                        "Viscosity at rest");
      prm.declare_entry("tauy",
                        "0.00976",
                        Patterns::Double(),
                        "Yield Stress");
      prm.declare_entry("m", "1000.0", Patterns::Double(), "Papanastasiou parameter");
      prm.declare_entry("k", "0.00241", Patterns::Double(), "Consistency");
      prm.declare_entry("n", "0.48", Patterns::Double(), "Power parameter");
    }
    prm.leave_subsection();
  }

  void
  HerschelParameters::parse_parameters(ParameterHandler &prm)//Insert by Mo
  {
    prm.enter_subsection("herschel");
    {
      viscosity_0   = prm.get_double("viscosity_0");
      tauy = prm.get_double("tauy");
      m        = prm.get_double("m");
      k             = prm.get_double("k");
      n             = prm.get_double("n");
    }
  ...
  NonNewtonian::declare_parameters(ParameterHandler &prm)
  {
    prm.enter_subsection("non newtonian");
    {
      prm.declare_entry("model",
                        "carreau",
                        Patterns::Selection("power-law|carreau|herschel"),//Insert by Mo
                        "Non newtonian model "
                        "Choices are <power-law|carreau|herschel>.");//Insert by Mo
      powerlaw_parameters.declare_parameters(prm);
      carreau_parameters.declare_parameters(prm);
      herschel_parameters.declare_parameters(prm);//Insert by Mo
    }
    prm.leave_subsection();
  }
  ...
  NonNewtonian::parse_parameters(ParameterHandler &prm)
  {
    prm.enter_subsection("non newtonian");
    {
      ...
      else if (op == "herschel")//Insert by Mo
        {
          model = Model::herschel;
          herschel_parameters.parse_parameters(prm);
        }
      else if (op == "carreau")
        {
          model = Model::carreau;

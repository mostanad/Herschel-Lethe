  ...
  if (!physical_properties.non_newtonian_flow)
    ...
    return std::make_shared<PowerLaw>(
      ...
      physical_properties.non_newtonian_parameters.powerlaw_parameters
        .shear_rate_min);
  else if (physical_properties.non_newtonian_parameters.model == //Insert by Mo
           Parameters::NonNewtonian::Model::herschel)
    return std::make_shared<Herschel>(
      physical_properties.non_newtonian_parameters.herschel_parameters.viscosity_0,
      physical_properties.non_newtonian_parameters.herschel_parameters.tauy,
      physical_properties.non_newtonian_parameters.herschel_parameters.m,
      physical_properties.non_newtonian_parameters.herschel_parameters.k,
      physical_properties.non_newtonian_parameters.herschel_parameters.n);
  else // if (physical_properties.non_newtonian_parameters.model ==
...
double
Herschel::value(const std::map<field, double> &field_values)
{
  const double shear_rate_magnitude = field_values.at(field::shear_rate);
...
void
Herschel::vector_value(const std::map<field, std::vector<double>> &field_vectors,
                      std::vector<double> &property_vector)
{
  const auto shear_rate_magnitude = field_vectors.at(field::shear_rate);
...
double
Herschel::jacobian(const std::map<field, double> &field_values, const field id)
{
  if (id == field::shear_rate)
    return this->numerical_jacobian(field_values, field::shear_rate);
  else
